window.addEventListener('DOMContentLoaded', (event) => {
    ...
    // Hide sidebar
    let hideSidebar = document.getElementById('elHideSidebar');

    hideSidebar.addEventListener('click', () => {
        let shown = hideSidebar.getAttribute('aria-selected');
        if (shown) {
            document.body.classList.remove('m-full');
            hideSidebar.removeAttribute('aria-selected');
        } else {
            document.body.classList.add('m-full');
            hideSidebar.setAttribute('aria-selected', 'true');
        }
    });
